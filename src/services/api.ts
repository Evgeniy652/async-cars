...
export const getCarById = async (id: string): Promise<Car> => (await fetch(`${BASE_URL}/garage/${id}`)).json();
...
export const getStartEngine = async (id: number): Promise<Engine> =>
  (await fetch(`${BASE_URL}/engine?id=${id}&status=started`, { method: 'PATCH' })).json();
...
export const getDriveStatus = async (id: number): Promise<{ success: boolean }> => {
  const res = await fetch(`${BASE_URL}/engine?id=${id}&status=drive`, { method: 'PATCH' }).catch();
...
export const getWinners = async ({
  ...
  const response = await fetch(`${BASE_URL}/winners?_page=${page}&_limit=${limit}${getSortOrder(sort, order)}`);
...
export const getWinner = async (id: number): Promise<Winner> => (await fetch(`${BASE_URL}/winners/${id}`)).json();

export const getWinnerStatus = async (id: number): Promise<number> => (await fetch(`${BASE_URL}/winners/${id}`)).status;
